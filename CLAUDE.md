...
- **Tests** (Vitest): `npm test` = offline unit suite (Supabase/fetch mocked via `src/test/mocks.ts`; covers the db repository, export/import, geocoding parsers, deep links, convention tripwires). `npm run test:rls` = live read-only anon requests verifying the RLS privacy boundary — run it after any schema/RLS change (it skips with a warning when the free-tier project is paused). A committed pre-push hook (`.githooks/`, wired by the `prepare` script) runs `tsc -b` + unit tests; bypass with `--no-verify` only for emergencies. Test fixtures must use invented data (see the real-data rule above). Known gaps: `image.ts` (needs a real canvas), `legacy.ts` (one-time migration), React components/Leaflet (manual browser verification).
- Typecheck (`npx tsc -b` — plain `tsc --noEmit` checks nothing against the solution-style root tsconfig) + build before committing; commit messages explain the why; pushing `main` deploys. Confirm the deploy concluded `success` and spot-check the live site (anon sees nothing private).
